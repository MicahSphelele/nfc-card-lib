...
public enum EmvCardScheme {
     ...
     * @param regex Card regex
     * @param pAids AID or RID
     */

    EmvCardScheme(final String scheme, final String regex, final String... pAids) {
        aids = pAids;
     ...
     */
    public static EmvCardScheme getCardTypeByCardNumber(final String cardNumber) {
        EmvCardScheme ret = EmvCardScheme.UNKNOWN;
        if (cardNumber != null) {
            for (EmvCardScheme val : EmvCardScheme.values()) {
                if (val.pattern != null && val.pattern.matcher(StringUtils.deleteWhitespace(cardNumber)).matches()) {
                    ret = val;
                    break;
                }
            }
